...
def newMUDA(buyers:list, numOfItems:int):
    ...
    print("buyers_sorted_by_2")
    for b in buyers_sorted_by_2: print("  ",b)

    # calculate price for 2 units:
    price_2 = 0
    for b in buyers_sorted_by_2:
        if mapIndexToQuantity.get(b.index,0)!=2: #==0:#
    ...
    for b in vbuyers_sorted_by_1:
        if mapIndexToQuantity.get(b[2],0)!=1: #==0: #
            price_1 = b[1]
            break
...
if __name__ == "__main__":
    ...
    # seed = 115423 # bad for "==0,==0"
    # seed = 353511 # bad for "!=1,!=2"
    ...
    numOfRealBuyers = 6
    traders = []
    for i in range(numOfRealBuyers):
    ...
    print(newMUDA(traders, numOfItems=numOfSellers))
