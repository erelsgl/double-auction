...
def randomTradeWithExogeneousPrice(traders:list, price:float)->tuple:
	...
	>>> s1 = Trader.Seller([[5,200]])
	>>> s2 = Trader.Seller([[4,100],[3,300]])
	>>> randomTradeWithExogeneousPrice([b1,b2,s1,s2],50)[0:3]
	(0, 0, 0)
	>>> randomTradeWithExogeneousPrice([b1,b2,s1,s2],100)[0:3]
	(1, 1, 4)
	>>> randomTradeWithExogeneousPrice([b1,b2,s1,s2],150)[0:3]
	(1, 1, 4)
	>>> randomTradeWithExogeneousPrice([b1,b2,s1,s2],200)[0:3]
	(2, 2, 8)
	"""	
	...
	numOfBuyers = 0
	numOfSellers = 0
	partialBuyer = partialSeller = False
	totalUnitsTraded = 0
	gainFromTrade = 0
	while virtualBuyers and virtualSellers:
		...
		partialBuyer = partialSeller = False
		if currentDemand > currentSupply:   # virtual seller goes home happy, virtual buyer remains
			numOfSellers += 1
			partialBuyer = True
			virtualBuyers.append((currentDemand-currentSupply, currentDemandValue))
		elif currentDemand < currentSupply: # virtual buyer goes home happy, virtual seller remains
			numOfBuyers += 1
			partialSeller = True
			virtualSellers.append((currentSupply-currentDemand, currentSupplyValue))
		else:   # Both buyer and seller go home happy
			numOfBuyers += 1
			numOfSellers += 1
		totalUnitsTraded += units
		gainFromTrade += units*(currentDemandValue - currentSupplyValue)
	return (numOfBuyers+partialBuyer, numOfSellers+partialSeller, totalUnitsTraded, gainFromTrade)
...
def winningAndLosingTraders(traders:list, quota:int)->tuple:
	"""
	INPUT: a list with items of type (quantity,value,index).
	
	OUTPUT: two lists: winning items and losing items. The total quantity of the winning items is at most the quota.
...
def MIDA(traders:list, lottery=True) -> (int,float):
	"""
	Run the Multi-Item-Double-Auction mechanism.
	INPUT: a list of Trader objects, each of which represents valuations with decreasing marginal returns.
		* lottery - handle excess demand/supply using a lottery (if False - use Vickrey)
	...
	if lottery:
		(numOfBuyersLeft, numOfSellersLeft, sizeLeft, gainLeft) = randomTradeWithExogeneousPrice(tradersLeft, priceRight)
		(numOfBuyersRight, numOfSellersRight, sizeRight, gainRight) = randomTradeWithExogeneousPrice(tradersRight, priceLeft)
		return (sizeRight+sizeLeft, gainRight+gainLeft, gainRight+gainLeft)
	else:
		...
		return (sizeRight+sizeLeft, tradersGainRight+tradersGainLeft, totalGainRight+totalGainLeft)
MIDA.LOG = False
